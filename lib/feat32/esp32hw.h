...
void GoingToSleep(byte Time_minutes = 0, unsigned long currUTime = 0 ) {
  ...
  // Configure Wake Up
    if ( Ext1WakeUP>=0 && (Time_minutes ==0 || Time_minutes > 5) ) {
        // const uint64_t ext_wakeup_pin_1_mask = 1ULL << Ext1WakeUP;      // -1 Warning during compilling
        const uint64_t ext_wakeup_pin_1_mask = uint64_t(Ext1WakeUP);
...
String Flash_Size() {
    return String(ESP.getFlashChipSize()) + "MB";
}
